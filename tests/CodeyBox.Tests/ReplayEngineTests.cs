...
public sealed class ReplayEngineTests
{
    ...
    [Fact]
    public async Task Replay_FailsWithWaitTimeout_WhenScreenshotStepNeverSettles()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
    ...
    public async Task Replay_RunsInParallel_AgainstIndependentSandboxes_IncludingAssertions()
    {
        ...
        var engine = new ReplayEngine(locator: TestLocator());
    ...
    public async Task Replay_RunsInParallel_ForcesOverlappingAssertionVerification()
    {
        ...
        var engine = new ReplayEngine(locator: TestLocator(), assertions: verifier);
    ...
    [Fact]
    public async Task Replay_ScreenshotStepWithoutAssertion_VerifiesRecordedCheckpoint()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotB);
        var entry = new TraceEntry
        {
    ...
    [Fact]
    public async Task Replay_ActionStepWithoutAssertion_VerifiesRecordedCheckpoint()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotB)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        ...
        var result = await NewEngineFor(sandbox).ReplayAsync(sandbox, MakeTrace(entry));
    ...
    public async Task Replay_SanitizesUntrustedAccessibilityTextInDiagnostics()
    {
        ...
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) =>
            {
                probeCalls++;
                return Accessible("button", "Checkout");
            },
        };
        var trace = MakeTrace(ClickEntry(seq: 1,
            region: new TraceBoundingRegion { X = 150, Y = 80, Width = 40, Height = 20 },
    ...
    public async Task Replay_FocusesRelocatedTargetBeforeDispatchingKey()
    {
        ...
        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
        var locatedTarget = result.Steps[0].LocatedTarget;
    ...
    [Fact]
    public async Task Replay_DispatchesBareScroll_WhenViewportVisualDescriptorMatches()
    {
        var screenshot = BuildRgbPng(2, 2, [0, 255, 0, 0, 0, 255, 255, 0, 0, 255, 255, 255]);
        var sandbox = new ScriptedSandbox(screenshot)
        {
        ...
        var entry = new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                ...
                TargetDescriptor = new TraceTargetDescriptor
                {
                    Visual = new TraceVisualDescriptor
                    {
                        Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 2, Height = 2 },
                        ClickOffsetX = 1,
                        ClickOffsetY = 1,
        ...
        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
        Assert.Contains(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Move && e.X == 1 && e.Y == 1);
        var scroll = Assert.Single(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Scroll);
    ...
    public async Task Replay_LocatorFindsTargetViaRingScan_WhenCentreMisses()
    {
        ...
        Assert.Equal(158, located.Region.X);
        Assert.Equal(80, located.Region.Y);
        Assert.Equal(40, located.Region.Width);
        Assert.Equal(20, located.Region.Height);
    }

    [Fact]
    public async Task AccessibilityLocator_ReturnsNull_WhenOnlyRecordedClickOffsetPointProbeMatches()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (x, y) =>
    ...
    [Fact]
    public async Task Replay_WaitsForScrollSettle_BeforeReachabilityRelocates()
    {
        var settled = false;
        var locateCalls = 0;
        var locator = new ScriptedLocator(target =>
        ...
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (x, y) => x == 220 && y == 600 ? Accessible("button", "Submit") : null,
        };
        var trace = MakeTrace(
            ClickEntry(seq: 1, region: new TraceBoundingRegion { X = 200, Y = 900, Width = 40, Height = 20 },
                accessibility: new TraceAccessibilityDescriptor { Role = "button", Name = "Submit" }));
        var clock = new FakeTimeProvider(FrozenNow);
        var wait = new CallbackWait(() => settled = true);
        var engine = new ReplayEngine(
            bridgeFactory: () => new ComputerUseBridge(timeProvider: clock),
            locator: locator,
            visualWait: wait,
        ...
        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
        Assert.True(wait.Calls > 0);
        var click = Assert.Single(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Click);
        Assert.Equal(600, click.Y);
    }

    [Fact]
    public async Task Reachability_FailsOffScreen_WhenPostScrollSettleTimesOut()
    {
        var locatorCalls = 0;
        var locator = new ScriptedLocator(target =>
        {
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.OffScreen, result.FailedStep!.FailureKind);
        Assert.Contains("screen did not settle after scrolling", result.FailedStep.Diagnostic);
        Assert.Equal(StableScreenshotA, result.FailedStep.DiagnosticScreenshotPng);
        Assert.Contains(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Scroll);
        Assert.Equal(1, locatorCalls);
    }
    ...
    public async Task Replay_FailsWithOccluded_WhenAccessibilityProbeReturnsNullAtLocatedCentre()
    {
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.Occluded, result.FailedStep!.FailureKind);
        Assert.Contains("did not answer the top-most accessibility probe", result.FailedStep.Diagnostic);
    }
    ...
    [Fact]
    public async Task Replay_FailsWithActionFailed_WhenScreenshotWaitCannotCapture()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
    ...
    [Fact]
    public async Task Replay_FailsWithActionFailed_WhenActionStepVisualWaitThrows()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        ...
        var engine = new ReplayEngine(
            bridgeFactory: () => new ComputerUseBridge(timeProvider: clock),
            locator: TestLocator(),
            visualWait: new ThrowingVisualWait(new InvalidOperationException("post-input wait failed")),
            timeProvider: clock);
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.ActionFailed, result.FailedStep!.FailureKind);
        Assert.Contains("visual wait failed", result.FailedStep.Diagnostic);
        Assert.Contains("post-input wait failed", result.FailedStep.Diagnostic);
        Assert.Contains(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Click);
    }

    // ------------------------------------------------------------------
    // ScreenshotStabilityWait: predicate stability + capture-exception propagation
    // ------------------------------------------------------------------

    [Fact]
    public async Task VisualWait_RequiresStabilityAfterPredicateMatch()
    {
        // The first captured frame already satisfies the predicate, but the
        // wait still needs a second identical matching frame before returning.
        var sandbox = new ScriptedSandbox(StableScreenshotA);
        var calls = 0;
        ...
        var settled = await wait.WaitAsync(
            sandbox,
            predicate: (current, _) => Task.FromResult(current.Length == 2 && current[0] == 0xCA),
            options,
    ...
    [Fact]
    public async Task VisualWait_PropagatesScreenshotException()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
        sandbox.GetScreenshot = _ => Task.FromException<byte[]>(new InvalidOperationException("framebuffer failed"));
        ...
        var ex = await Assert.ThrowsAsync<InvalidOperationException>(
            () => wait.WaitAsync(sandbox, predicate: null, options, CancellationToken.None));
        Assert.Contains("framebuffer failed", ex.Message);
    }

    [Fact]
    public async Task VisualWait_UsesDecodedPixels_NotPngBytes_ForStability()
    {
        var first = BuildRgbPng(1, 1, [12, 34, 56]);
        var second = BuildRgbPngWithTextChunk(1, 1, [12, 34, 56], "encoder", "different");
        Assert.NotEqual(first, second);
        var frames = new Queue<byte[]>(new[] { first, second });
        var sandbox = new ScriptedSandbox(first);
        sandbox.GetScreenshot = _ => Task.FromResult(frames.Count > 0 ? frames.Dequeue() : second);
        ...
        {
            VisualWaitPollInterval = TimeSpan.FromMilliseconds(50),
            VisualWaitTimeout = TimeSpan.FromSeconds(5),
            StableFrameCount = 2,
        };
    ...
    [Fact]
    public async Task DefaultAssertionVerifier_VisualMatch_UsesDecodedPixelsByDefault()
    {
        var first = BuildRgbPng(1, 1, [12, 34, 56]);
        var second = BuildRgbPngWithTextChunk(1, 1, [12, 34, 56], "encoder", "different");
        Assert.NotEqual(first, second);
        ...
        var diag = await verifier.VerifyAsync(
            assertion: new TraceAssertion { Kind = "visual-match" },
            currentScreenshotPng: second,
            recordedScreenshotPng: first,
            accessibilitySnapshotJson: null,
            ct: CancellationToken.None);
    ...
    public async Task VisualWait_ReturnsNull_WhenPredicateNeverMatches()
    {
        ...
        var settled = await wait.WaitAsync(
            sandbox,
            predicate: (_, _) => Task.FromResult(false),
            options,
            CancellationToken.None);
    ...
    {
        // The expected screenshot never appears. The wait still polls through
        // the verifier, but the public result must report the verifier's
        // visual mismatch diagnostic rather than collapsing to WaitTimeout.
        var sandbox = new ScriptedSandbox(StableScreenshotB)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        };
    ...
    [Fact]
    public async Task Replay_InlineVisualMatch_UsesCustomComparerDuringWait()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotB)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        ...
        var entry = ClickEntry(seq: 1,
            region: new TraceBoundingRegion { X = 150, Y = 80, Width = 40, Height = 20 },
            accessibility: new TraceAccessibilityDescriptor { Role = "button", Name = "Login" },
            observationScreenshot: StableScreenshotA) with
        {
            Assertion = new TraceAssertion { Kind = "visual-match" },
        };
        var verifier = new DefaultAssertionVerifier(
        ...
        var result = await engine.ReplayAsync(sandbox, MakeTrace(entry), options);

        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
    }

    [Fact]
    public async Task Replay_TextAssertionPollsUntilExpectedAccessibilityAppears()
    {
        var treeCalls = 0;
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
            GetAccessibilityTree = _ =>
            {
                treeCalls++;
                return Task.FromResult<string?>(treeCalls < 3
                    ? """{"tree":[{"text":"Loading"}]}"""
                    : """{"tree":[{"text":"Welcome"}]}""");
            },
        };
        ...
        var task = engine.ReplayAsync(sandbox, MakeTrace(entry), options);
        for (var i = 0; i < 10 && !task.IsCompleted; i++)
        {
            clock.Advance(TimeSpan.FromMilliseconds(60));
            await Task.Yield();
    ...
    [Fact]
    public async Task Replay_FailsRecorderProducedTargetlessScroll()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
        var entry = new TraceEntry
        {
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.NotFound, result.FailedStep!.FailureKind);
        Assert.Contains("blind wheel event", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public async Task Replay_TargetScopedScrollMovesToLocatedTargetBeforeScrolling()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) => Accessible("list", "tasks"),
        ...
        var entry = new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                InputEvents = [new SandboxInputEvent { Type = SandboxInputEventType.Scroll, Y = -4 }],
                Kind = "scroll",
                TargetDescriptor = new TraceTargetDescriptor
                {
                    Accessibility = new TraceAccessibilityDescriptor { Role = "list", Name = "tasks" },
                    Visual = new TraceVisualDescriptor { Region = new TraceBoundingRegion { X = 10, Y = 20, Width = 100, Height = 80 } },
                },
            },
            Observation = new TraceObservation { ScreenshotPng = null, CapturedAt = FrozenNow },
        };
        ...
        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
        Assert.True(sandbox.RecordedInputEvents.Count >= 2);
        Assert.Equal(SandboxInputEventType.Move, sandbox.RecordedInputEvents[0].Type);
        Assert.Equal(60, sandbox.RecordedInputEvents[0].X);
        Assert.Equal(60, sandbox.RecordedInputEvents[0].Y);
        Assert.Equal(SandboxInputEventType.Scroll, sandbox.RecordedInputEvents[1].Type);
        Assert.Equal(-4, sandbox.RecordedInputEvents[1].Y);
    }

    [Fact]
    public async Task Replay_FailsKeyboardInputWithoutTargetDescriptorUnlessMarkedGlobal()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
        var trace = MakeTrace(
            new TraceEntry
            ...
            new TraceEntry
            {
                ...
                Action = new TraceAction
                {
                    InputEvents = [new SandboxInputEvent { Type = SandboxInputEventType.Type, Text = "hello" }],
                    Kind = "type",
                    TargetDescriptor = new TraceTargetDescriptor
                    {
                        Visual = new TraceVisualDescriptor { Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 0, Height = 0 } },
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.NotFound, result.FailedStep!.FailureKind);
        Assert.Contains("no focus target descriptor", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }
    ...
    [Fact]
    public async Task Replay_FallsBackToVisualEvidence_WhenAccessibilitySupportIsAbsent()
    {
        var screenshot = BuildRgbPng(1, 1, [0, 255, 0]);
        var sandbox = new ScriptedSandbox(screenshot)
        {
            AccessibilityAtPoint = (_, _) => null,
        ...
        var result = await NewEngineFor(sandbox).ReplayAsync(sandbox, trace);

        Assert.True(result.Passed, result.FailedStep?.Diagnostic);
        Assert.Equal(LocatedTargetEvidence.Visual, result.Steps[0].LocatedTarget!.Evidence);
        Assert.Contains(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Click);
    }

    [Fact]
    public async Task Replay_FailsOccluded_WhenAccessibilityTaggedTargetOnlyHasVisualFallback()
    {
        var screenshot = BuildRgbPng(1, 1, [0, 255, 0]);
        var sandbox = new ScriptedSandbox(screenshot)
        {
        ...
        var trace = MakeTrace(new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                ...
                TargetDescriptor = new TraceTargetDescriptor
                {
                    Accessibility = new TraceAccessibilityDescriptor { Role = "button", Name = "Checkout" },
                    Visual = new TraceVisualDescriptor
                    {
                        Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 1, Height = 1 },
                        SourceScreenshotPng = screenshot,
    ...
    [Fact]
    public async Task Replay_ScrollSearchesVisualOnlyTarget_WhenRecordedRegionIsOffScreen()
    {
        var source = BuildSingleColumnPngWithTargetAtY(900);
        ...
        var trace = MakeTrace(new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                ...
                TargetDescriptor = new TraceTargetDescriptor
                {
                    ...
                    Visual = new TraceVisualDescriptor
                    {
                        Region = new TraceBoundingRegion { X = 0, Y = 900, Width = 1, Height = 1 },
    ...
    [Fact]
    public async Task Replay_DoesNotScrollSearchForInViewportVisualMisses()
    {
        ...
        sandbox.GetScreenshot = _ => Task.FromResult(scrollCount >= 2 ? source : beforeScroll);
        var trace = MakeTrace(new TraceEntry
        {
            Sequence = 1,
            Timestamp = FrozenNow,
    ...
    [Fact]
    public async Task Replay_WaitsForScrollSettleAfterOffScreenVisualLocatorMiss()
    {
        var source = BuildSingleColumnPngWithTargetAtY(900);
        var afterScroll = BuildRgbPng(1, 1, [0, 255, 0]);
        var beforeScroll = BuildRgbPng(1, 1, [255, 0, 0]);
        ...
        sandbox.GetScreenshot = _ => Task.FromResult(scrolled && settled ? afterScroll : beforeScroll);
        var trace = MakeTrace(new TraceEntry
        {
            Sequence = 1,
            Timestamp = FrozenNow,
    ...
    [Fact]
    public async Task Replay_FailsOffScreen_WhenOffScreenVisualMissAllowsNoScrollAttempts()
    {
        var source = BuildSingleColumnPngWithTargetAtY(900);
        var beforeScroll = BuildRgbPng(1, 1, [255, 0, 0]);
        var sandbox = new ScriptedSandbox(beforeScroll)
        ...
        var trace = MakeTrace(new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                InputEvents = [new SandboxInputEvent { Type = SandboxInputEventType.Click, X = 0, Y = 900 }],
                Kind = "click",
                TargetDescriptor = new TraceTargetDescriptor
                {
                    Visual = new TraceVisualDescriptor
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.OffScreen, result.FailedStep!.FailureKind);
        Assert.Contains("no scroll attempts are allowed", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public async Task Replay_FailsOffScreen_WhenVisualMissScrollSearchWaitTimesOut()
    {
        var source = BuildSingleColumnPngWithTargetAtY(900);
        var beforeScroll = BuildRgbPng(1, 1, [255, 0, 0]);
        var sandbox = new ScriptedSandbox(beforeScroll)
        ...
        Assert.Contains("screen did not settle after scrolling", result.FailedStep.Diagnostic);
        Assert.Contains(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Scroll);
        Assert.DoesNotContain(sandbox.RecordedInputEvents, e => e.Type == SandboxInputEventType.Click);
    }

    [Fact]
    public async Task Replay_FailsActionFailed_WhenVisualMissScrollSearchDispatchThrows()
    {
        var source = BuildSingleColumnPngWithTargetAtY(900);
        var beforeScroll = BuildRgbPng(1, 1, [255, 0, 0]);
        var sandbox = new ScriptedSandbox(beforeScroll)
        {
            ...
            {
                if (events.Any(e => e.Type == SandboxInputEventType.Scroll))
                    throw new InvalidOperationException("scroll input rejected");
            },
        };
        var trace = MakeTrace(new TraceEntry
        {
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.ActionFailed, result.FailedStep!.FailureKind);
        Assert.Contains("scroll search for off-screen visual target failed", result.FailedStep.Diagnostic);
        Assert.Contains("scroll input rejected", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public async Task Replay_DrivesClick_WhenVisualOnlyTargetInsideAccessibleContainer()
    {
        var screenshot = BuildRgbPng(1, 1, [0, 255, 0]);
        var sandbox = new ScriptedSandbox(screenshot)
        {
            AccessibilityAtPoint = (_, _) => Accessible("document", "JobTrack"),
        };
        var trace = MakeTrace(new TraceEntry
        {
            Sequence = 1,
    ...
    [Fact]
    public async Task Replay_FailsWithOccluded_WhenAccessibleTargetPixelsAreCovered()
    {
        var recorded = BuildRgbPng(1, 1, [0, 255, 0]);
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.Occluded, result.FailedStep!.FailureKind);
        Assert.Contains("visible pixels no longer match", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public async Task Replay_FailsWithOccluded_WhenAccessibleTargetVisualEvidenceIsUnverifiable()
    {
        var sandbox = new ScriptedSandbox([1, 2, 3])
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        };
        var trace = MakeTrace(new TraceEntry
        {
            ...
            Action = new TraceAction
            {
                InputEvents = [new SandboxInputEvent { Type = SandboxInputEventType.Click, X = 10, Y = 10 }],
                Kind = "click",
                TargetDescriptor = new TraceTargetDescriptor
                {
                    Accessibility = new TraceAccessibilityDescriptor { Role = "button", Name = "Login" },
                    Visual = new TraceVisualDescriptor
                    {
                        Region = new TraceBoundingRegion { X = 10, Y = 10, Width = 5, Height = 5 },
                        SourceScreenshotPng = [9, 9, 9],
                    },
                },
            },
            Observation = new TraceObservation { ScreenshotPng = null, CapturedAt = FrozenNow },
        });
        var locator = new ScriptedLocator(_ => new LocatedTarget
        {
            CenterX = 10,
            CenterY = 10,
            Region = new TraceBoundingRegion { X = 10, Y = 10, Width = 5, Height = 5 },
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.Occluded, result.FailedStep!.FailureKind);
        Assert.Contains("could not be verified", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public void DescriptorVisualTargetVerifier_VerifiesTemplateAtLocatedRegion()
    {
        var green = BuildRgbPng(1, 1, [0, 255, 0]);
        var current = BuildRgbPng(3, 1, [255, 0, 0, 0, 255, 0, 0, 0, 255]);
    ...
    [Fact]
    public void DescriptorVisualTargetVerifier_VerifiesSourceCropAtLocatedRegion()
    {
    ...
    [Fact]
    public void DescriptorVisualTargetVerifier_VerifiesSourceCropAtClickAlignedOriginForAccessibilityBounds()
    {
        var source = BuildRgbPng(3, 1, [255, 0, 0, 0, 255, 0, 0, 0, 255]);
        var current = BuildRgbPng(5, 1, [255, 255, 255, 0, 255, 0, 255, 255, 255, 255, 255, 255, 255, 255, 255]);
        var visual = new TraceVisualDescriptor
        {
            Region = new TraceBoundingRegion { X = 1, Y = 0, Width = 1, Height = 1 },
            ClickOffsetX = 0,
        ...
        var target = new LocatedTarget
        {
            CenterX = 1,
            CenterY = 0,
            Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 5, Height = 1 },
    ...
    [Fact]
    public async Task Replay_FailsWithOccluded_WhenVisualOnlyTopMostProbeReturnsNullAndPixelsMismatch()
    {
        var recorded = BuildRgbPng(1, 1, [0, 255, 0]);
        var current = BuildRgbPng(1, 1, [255, 0, 0]);
        var sandbox = new ScriptedSandbox(current)
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.Occluded, result.FailedStep!.FailureKind);
        Assert.Contains("pixels no longer match", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }

    [Fact]
    public async Task Replay_FailsWithOccluded_WhenVisualOnlyTopMostProbeReturnsNullAndSignatureUnverifiable()
    {
        var sandbox = new ScriptedSandbox([1, 2, 3])
        {
            AccessibilityAtPoint = (_, _) => null,
        ...
        Assert.False(result.Passed);
        Assert.Equal(ReplayFailureKind.Occluded, result.FailedStep!.FailureKind);
        Assert.Contains("no verifiable current visual signature", result.FailedStep.Diagnostic);
        Assert.Empty(sandbox.RecordedInputEvents);
    }
    ...
    public async Task ReplayEngine_AcceptsCustomAccessibilityMatcher()
    {
        ...
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            ...
            AccessibilityTreeJson = """
                {"role":"button","name":"Different","bounds":{"x":150,"y":80,"width":40,"height":20}}
                """,
        };
        var trace = MakeTrace(
            ClickEntry(seq: 1, region: new TraceBoundingRegion { X = 150, Y = 80, Width = 40, Height = 20 },
    ...
    [Fact]
    public async Task Replay_PublicGuardPathsRejectNullArguments()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA);
        var descriptor = new TraceTargetDescriptor
        {
            Accessibility = new TraceAccessibilityDescriptor { Role = "button", Name = "Login" },
            Visual = new TraceVisualDescriptor { Region = new TraceBoundingRegion { X = 10, Y = 20, Width = 30, Height = 40 } },
        };
        var options = new ReplayOptions();
        var target = new LocatedTarget
        {
        ...
        var accessibilityLocator = new AccessibilityElementLocator();
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => accessibilityLocator.LocateAsync(null!, descriptor, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => accessibilityLocator.LocateAsync(sandbox, null!, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => accessibilityLocator.LocateAsync(sandbox, descriptor, null!, CancellationToken.None));

        Assert.Throws<ArgumentNullException>(() => new VisualSignatureElementLocator((IElementLocator)null!));
        var visualLocator = new VisualSignatureElementLocator();
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => visualLocator.LocateAsync(null!, descriptor, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => visualLocator.LocateAsync(sandbox, null!, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => visualLocator.LocateAsync(sandbox, descriptor, null!, CancellationToken.None));

        Assert.Throws<ArgumentNullException>(() => new ReachabilityChecker(null!));
        var reachability = new ReachabilityChecker(new ComputerUseBridge());
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => reachability.EnsureReachableAsync(null!, target, descriptor, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => reachability.EnsureReachableAsync(sandbox, null!, descriptor, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => reachability.EnsureReachableAsync(sandbox, target, null!, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => reachability.EnsureReachableAsync(sandbox, target, descriptor, null!, CancellationToken.None));

        var wait = new ScreenshotStabilityWait();
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => wait.WaitAsync(null!, predicate: null, options, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(
            () => wait.WaitAsync(sandbox, predicate: null, null!, CancellationToken.None));

        Assert.Throws<ArgumentNullException>(() => new DefaultAssertionVerifier(null!));
        Assert.Throws<ArgumentNullException>(() => new DefaultAssertionVerifier(
            new Dictionary<string, byte[]>(StringComparer.Ordinal),
            null!));
        var verifier = new DefaultAssertionVerifier();
        await Assert.ThrowsAsync<ArgumentNullException>(() => verifier.VerifyAsync(
            null!,
            StableScreenshotA,
            StableScreenshotA,
            null,
        ...
        Assert.Throws<ArgumentNullException>(() => new CompositeElementLocator((IReadOnlyList<IElementLocator>)null!));
        Assert.Throws<ArgumentNullException>(() => DefaultAccessibilityMatcher.Instance.Matches(null!, descriptor.Accessibility!));
        Assert.Throws<ArgumentNullException>(() => DefaultAccessibilityMatcher.Instance.Matches(new SandboxAccessibilitySnapshot(), null!));
        Assert.Throws<ArgumentNullException>(() => DefaultAccessibilityMatcher.Instance.HasAnyAccessibilitySignal(null!));

        var nullBridgeEngine = new ReplayEngine(bridgeFactory: () => null!);
        await Assert.ThrowsAsync<InvalidOperationException>(
            () => nullBridgeEngine.ReplayAsync(sandbox, MakeTrace()));
    }
    ...
    public async Task Replay_UsesHarnessSession_AndDisposesIt()
    {
        ...
        var trace = MakeTrace(
            ClickEntry(seq: 1, region: new TraceBoundingRegion { X = 150, Y = 80, Width = 40, Height = 20 },
                accessibility: new TraceAccessibilityDescriptor { Role = "button", Name = "Login" }));
        var engine = new ReplayEngine(locator: TestLocator(), visualWait: new ImmediateWait());
    ...
    [Fact]
    public async Task AccessibilityLocator_TreeHitDoesNotTreatCropOffsetAsElementOffset()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityTreeJson = """
                {"nodes":[{"role":"slider","name":"Volume","bounds":{"x":400,"y":300,"width":100,"height":20}}]}
                """,
        };
        var descriptor = new TraceTargetDescriptor
        {
            ...
            Visual = new TraceVisualDescriptor
            {
                Region = new TraceBoundingRegion { X = 10, Y = 10, Width = 100, Height = 20 },
                ClickOffsetX = 12,
                ClickOffsetY = 5,
            },
        };
        ...
        Assert.NotNull(hit);
        Assert.Equal("accessibility-tree", hit.Source);
        Assert.Equal(450, hit.CenterX);
        Assert.Equal(310, hit.CenterY);
    }

    [Fact]
    public async Task AccessibilityLocator_TreeHitPreservesRecordedElementRelativeClickOffsetWhenBoundsWereRecorded()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityTreeJson = """
        ...
        var descriptor = new TraceTargetDescriptor
        {
            Accessibility = new TraceAccessibilityDescriptor
            {
                Role = "slider",
                Name = "Volume",
                Bounds = new TraceBoundingRegion { X = 10, Y = 10, Width = 100, Height = 20 },
            },
            Visual = new TraceVisualDescriptor
            {
                Region = new TraceBoundingRegion { X = 10, Y = 10, Width = 100, Height = 20 },
        ...
        Assert.NotNull(hit);
        Assert.Equal("accessibility-tree", hit.Source);
        Assert.Equal(412, hit.CenterX);
        Assert.Equal(305, hit.CenterY);
    }

    [Fact]
    public async Task AccessibilityLocator_ParsesProviderBoundsVariants()
    {
        var cases = new[]
        {
            (
                Json: """{"nodes":[{"role":"button","name":"Login","left":400,"top":300,"right":480,"bottom":330}]}""",
                X: 440,
                Y: 315),
            (
                Json: """{"nodes":[{"role":"button","name":"Login","bounds":[400,300,80,30]}]}""",
                X: 440,
                Y: 315),
            (
                Json: """{"nodes":[{"role":"button","name":"Login","bounds":{"x":"400","y":"300","width":"80","height":"30"}}]}""",
                X: 440,
                Y: 315),
            (
                Json: """{"nodes":[{"Role":"button","Name":"Login","Bounds":{"X":400,"Y":300,"Width":80,"Height":30}}]}""",
        ...
        foreach (var item in cases)
        {
            var sandbox = new ScriptedSandbox(StableScreenshotA)
            {
                AccessibilityTreeJson = item.Json,
                AccessibilityAtPoint = (_, _) => null,
            };
            ...
            Assert.NotNull(hit);
            Assert.Equal(item.X, hit.CenterX);
            Assert.Equal(item.Y, hit.CenterY);
        }
    }
    ...
    [Fact]
    public async Task AccessibilityLocator_ReturnsNull_WhenTreeHasNoBoundsEvenIfNearbyPointProbeMatches()
    {
        var calls = 0;
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
    ...
    [Fact]
    public async Task AccessibilityLocator_ReturnsNull_WhenTreeFetchThrows()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            GetAccessibilityTree = _ => Task.FromException<string?>(new InvalidOperationException("tree IPC failed")),
    ...
    [Fact]
    public async Task AccessibilityLocator_ReturnsNull_WhenTreeJsonIsMalformed()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityTreeJson = """{"nodes":[""",
            AccessibilityAtPoint = (_, _) => Accessible("button", "Login"),
        };
        var descriptor = new TraceTargetDescriptor
        {
    ...
    [Fact]
    public async Task AccessibilityLocator_ReturnsNull_WhenDuplicateTreeMatchesWouldNeedRecordedGeometry()
    {
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityTreeJson = """
    ...
    public async Task VisualSignatureLocator_FindsMovedTarget_FromSourceCrop()
    {
        ...
        var hit = await new VisualSignatureElementLocator(
                ...
                TesseractOcrTextLocator.Instance)
            .LocateAsync(sandbox, descriptor, new ReplayOptions(), CancellationToken.None);
    ...
    [Fact]
    public async Task VisualSignatureLocator_ToleratesSmallPixelDriftInSourceCrop()
    {
        var source = BuildRgbPng(2, 1, [10, 20, 30, 40, 50, 60]);
        var current = BuildRgbPng(2, 1, [12, 22, 31, 40, 50, 60]);
        var sandbox = new ScriptedSandbox(current);
        var descriptor = new TraceTargetDescriptor
        {
            Visual = new TraceVisualDescriptor
            {
                Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 2, Height = 1 },
                SourceScreenshotPng = source,
            },
        };
    ...
    [Fact]
    public async Task VisualSignatureLocator_ReturnsNull_WhenDuplicateTemplateMatchesWouldNeedRecordedGeometry()
    {
        var green = BuildRgbPng(1, 1, [0, 255, 0]);
    ...
    [Fact]
    public async Task VisualSignatureLocator_UsesTesseractOcrFallbackWhenAccessibilityTreeMisses()
    {
        ...
        Assert.NotNull(hit);
        Assert.Equal("visual-ocr", hit.Source);
        Assert.Equal(240, hit.CenterX);
        Assert.Equal(105, hit.CenterY);
        Assert.NotNull(capturedExec);
        Assert.Contains("tesseract", string.Join(" ", capturedExec!.Argv));
        Assert.False(string.IsNullOrWhiteSpace(capturedExec.Stdin));
    }
    ...
    [Fact]
    public void TesseractOcrLocator_ReturnsNullForDuplicateTextWithoutRecordedRegionDisambiguation()
    {
        var tsv =
            """
            level	page_num	block_num	par_num	line_num	word_num	left	top	width	height	conf	text
            5	1	1	1	1	1	10	10	40	20	96	Edit
            5	1	1	1	1	2	10	90	40	20	96	Edit
            """;

        var hit = TesseractOcrTextLocator.TryLocateFromTsv(tsv, "Edit");

        Assert.Null(hit);
    }

    [Fact]
    public void TesseractOcrLocator_UsesRecordedRegionToDisambiguateDuplicateText()
    {
        var tsv =
            """
            level	page_num	block_num	par_num	line_num	word_num	left	top	width	height	conf	text
        ...
        var visual = new TraceVisualDescriptor
        {
            Region = new TraceBoundingRegion { X = 8, Y = 86, Width = 44, Height = 28 },
            OcrText = "Edit",
        };

        var hit = TesseractOcrTextLocator.TryLocateFromTsv(tsv, "Edit", visual);
    ...
    [Fact]
    public void TesseractOcrLocator_LocatesMultiTokenTextFromAdjacentRows()
    {
        var tsv =
            """
            level	page_num	block_num	par_num	line_num	word_num	left	top	width	height	conf	text
            5	1	1	1	1	1	10	10	30	20	96	Sign
            5	1	1	1	1	2	45	10	20	20	95	In
            """;

        var hit = TesseractOcrTextLocator.TryLocateFromTsv(tsv, "Sign In");

        Assert.NotNull(hit);
        Assert.Equal("visual-ocr", hit.Source);
        Assert.Equal(37, hit.CenterX);
        Assert.Equal(20, hit.CenterY);
        Assert.Equal(new TraceBoundingRegion { X = 10, Y = 10, Width = 55, Height = 20 }, hit.Region);
    }

    [Fact]
    public void TesseractOcrLocator_DoesNotMatchSubstringInsideLongerToken()
    {
        var tsv =
            """
            level	page_num	block_num	par_num	line_num	word_num	left	top	width	height	conf	text
            5	1	1	1	1	1	10	10	60	20	96	Credit
            """;
    ...
    [Fact]
    public void TesseractOcrLocator_IgnoresLowConfidenceText()
    {
        var tsv =
            """
            level	page_num	block_num	par_num	line_num	word_num	left	top	width	height	conf	text
            5	1	1	1	1	1	10	10	40	20	42	Edit
            """;
    ...
    [Fact]
    public async Task TesseractOcrLocator_ThrowsForNullInputs()
    {
        var visual = new TraceVisualDescriptor
        {
            Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 1, Height = 1 },
        ...
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            TesseractOcrTextLocator.Instance.LocateTextAsync(null!, StableScreenshotA, visual, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            TesseractOcrTextLocator.Instance.LocateTextAsync(sandbox, null!, visual, CancellationToken.None));
        await Assert.ThrowsAsync<ArgumentNullException>(() =>
            TesseractOcrTextLocator.Instance.LocateTextAsync(sandbox, StableScreenshotA, null!, CancellationToken.None));
    }

    [Fact]
    public async Task TesseractOcrLocator_ReturnsNullWithoutShellingOutForBlankOcrText()
    {
        var execCalls = 0;
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
        ...
        var hit = await TesseractOcrTextLocator.Instance
            .LocateTextAsync(sandbox, StableScreenshotA, visual, CancellationToken.None);
    ...
    [Fact]
    public async Task TesseractOcrLocator_ReturnsNullWithoutShellingOutForEmptyOrOversizedScreenshot()
    {
        var execCalls = 0;
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
        ...
        var emptyHit = await TesseractOcrTextLocator.Instance
            .LocateTextAsync(sandbox, [], visual, CancellationToken.None);
        var oversizedHit = await TesseractOcrTextLocator.Instance
            .LocateTextAsync(sandbox, new byte[8 * 1024 * 1024 + 1], visual, CancellationToken.None);

        Assert.Null(emptyHit);
        Assert.Null(oversizedHit);
        Assert.Equal(0, execCalls);
    }

    [Fact]
    public async Task TesseractOcrLocator_ReturnsNullForNonzeroExitOrEmptyStdout()
    {
        var visual = new TraceVisualDescriptor
        {
            Region = new TraceBoundingRegion { X = 0, Y = 0, Width = 1, Height = 1 },
        ...
        var missingTesseract = new ScriptedSandbox(StableScreenshotA)
        {
            Exec = (_, _) => Task.FromResult(new SandboxExecResult(127, "", "not found")),
        };
        var emptyOutput = new ScriptedSandbox(StableScreenshotA)
        {
            Exec = (_, _) => Task.FromResult(new SandboxExecResult(0, "", "")),
        };

        var missingHit = await TesseractOcrTextLocator.Instance
            .LocateTextAsync(missingTesseract, StableScreenshotA, visual, CancellationToken.None);
        var emptyHit = await TesseractOcrTextLocator.Instance
            .LocateTextAsync(emptyOutput, StableScreenshotA, visual, CancellationToken.None);

        Assert.Null(missingHit);
        Assert.Null(emptyHit);
    }

    [Fact]
    public async Task Replay_DrivesClick_WhenTargetLocatedByOcrEvidence()
    {
        var ocrTarget = new LocatedTarget
        {
            CenterX = 240,
            CenterY = 105,
            Region = new TraceBoundingRegion { X = 180, Y = 90, Width = 120, Height = 30 },
            Source = "custom-ocr-provider",
            Confidence = 0.72,
            Evidence = LocatedTargetEvidence.Ocr,
        };
        var visualLocator = new VisualSignatureElementLocator(
            new ScriptedLocator(_ => null),
            new StubOcrTextLocator(ocrTarget));
        var probeCalls = 0;
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) =>
    ...
    [Fact]
    public async Task Replay_DrivesClick_WhenOcrTargetMatchesTopMostAccessibilityText()
    {
        var ocrTarget = new LocatedTarget
        {
            CenterX = 240,
        ...
        var sandbox = new ScriptedSandbox(StableScreenshotA)
        {
            AccessibilityAtPoint = (_, _) => Accessible("button", "Checkout button"),
        };
        var trace = MakeTrace(new TraceEntry
        {
            Sequence = 1,
    ...
    private static IElementLocator TestLocator()
        => new CompositeElementLocator(
            new AccessibilityElementLocator(),
            new PointProbeFixtureLocator(),
    ...
    private static byte[] BuildSingleColumnPngWithTargetAtY(int targetY)
    {
        var height = targetY + 1;
        var rgb = new byte[height * 3];
        ...
        var targetOffset = targetY * 3;
        rgb[targetOffset] = 0;
        rgb[targetOffset + 1] = 255;
        rgb[targetOffset + 2] = 0;
        return BuildRgbPng(1, height, rgb);
    }
    ...
    private static byte[] BuildRgbPngWithTextChunk(int width, int height, byte[] rgb, string keyword, string value)
    {
        if (rgb.Length != width * height * 3)
            throw new ArgumentException("RGB payload length does not match dimensions.", nameof(rgb));
        ...
        ihdr[9] = 2;
        WritePngChunk(png, "IHDR", ihdr);
        WritePngChunk(png, "tEXt", Encoding.ASCII.GetBytes(keyword + "\0" + value));
        WritePngChunk(png, "IDAT", CompressZlib(scanlines));
        WritePngChunk(png, "IEND", []);
        return png.ToArray();
    }
    ...
    private sealed class ScriptedSandbox : ISandbox
    {
        ...
        public Func<SandboxExec, CancellationToken, Task<SandboxExecResult>>? Exec { get; set; }
        ...
        public Task<SandboxExecResult> ExecAsync(SandboxExec exec, CancellationToken ct = default)
            => Exec?.Invoke(exec, ct) ?? Task.FromResult(new SandboxExecResult(0, "", ""));
    ...
    {
        public async Task<byte[]?> WaitAsync(ISandbox sandbox, Func<byte[], CancellationToken, Task<bool>>? predicate, ReplayOptions options, CancellationToken ct)
        {
            try
            {
                var screenshot = await sandbox.GetScreenshotAsync(ct);
                if (predicate is not null)
                    _ = await predicate(screenshot, ct);
                return screenshot;
            }
            catch { return null; }
        }
    ...
    private sealed class CallbackWait : IVisualWait
    {
        private readonly Action _onWait;
        public int Calls { get; private set; }

        public CallbackWait(Action onWait) => _onWait = onWait;
    ...
    private sealed class ImmediateNullWait : IVisualWait
    {
        ...
        public Task<byte[]?> WaitAsync(ISandbox sandbox, Func<byte[], CancellationToken, Task<bool>>? predicate, ReplayOptions options, CancellationToken ct)
    ...
    private sealed class PointProbeFixtureLocator : IElementLocator
    {
        private readonly IAccessibilityMatcher _matcher = DefaultAccessibilityMatcher.Instance;
            ...
            CancellationToken ct)
        {
            ...
            if (expected is null || !_matcher.HasAnyAccessibilitySignal(expected))
                return null;
            ...
            var point = await ProbeAsync(sandbox, cx, cy, expected, ct);
                 ...
                 radius += options.RingSearchStep)
            {
                foreach (var (dx, dy) in SquareRingOffsets(radius, options.RingSearchStep))
                {
                    var px = cx + dx;
                    var py = cy + dy;
                    var match = await ProbeAsync(sandbox, px, py, expected, ct);
        ...
        private async Task<SandboxAccessibilitySnapshot?> ProbeAsync(
            ISandbox sandbox,
            int x,
            int y,
            ...
            CancellationToken ct)
        {
            try
            {
                var snap = await sandbox.GetAccessibilityAtPointAsync(x, y, ct);
                return snap is not null && _matcher.Matches(snap, expected) ? snap : null;
            }
    ...
    private sealed class StubOcrTextLocator : IOcrTextLocator
    {
        private readonly LocatedTarget? _target;
        public StubOcrTextLocator(LocatedTarget? target) => _target = target;

...
public sealed class AgentNetworkToleranceProgramWiringTests
{
    ...
    public async Task ProgramInjectsConfiguredClaudeApiTimeoutIntoAcpTransport()
    {
        ...
        // The test fixture builds against an embedded placeholder bridge (the
        // publish script does not run in CI), so prime a fake binary so the
        // production placeholder gate doesn't short-circuit OpenAsync.

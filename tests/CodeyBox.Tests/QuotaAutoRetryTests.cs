...
public sealed class QuotaAutoRetryTests : IDisposable
{
        ...
        BuildPipeline(IAgentRunner agent, bool enabled = true, string? repoUrl = null)
    {
        ...
        var scheduler = new QuotaRetryScheduler(store, new InMemoryTaskQueue(), opts, NullLogger<QuotaRetryScheduler>.Instance, router, projects, null, webhooks, _time);
    ...
    public async Task Scheduler_PeriodicSweep_RetriesEligibleItems()
    {
        ...
        // Run sweep via private method using reflection or just make it internal
        ...
        Assert.Contains(webhooks.Events, e => e.Event == "workitem.auto_retry");
    }

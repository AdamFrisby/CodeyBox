...
public sealed class AgentPricingEndpointTests : IClassFixture<AgentPricingApiFactory>
{
    ...
    public async Task Get_ReturnsMergedRates_BundledCarriedThroughWithOperatorOverride()
    {
        ...
        Assert.Equal(1, counts.GetProperty("operatorOverrides").GetInt32());
...
{
    private readonly TestScratchDirectory _scratch = TestScratchDirectory.Create("codeybox-pricing-httptest-");
    private string _dbPath => _scratch.DbPath("pricing-httptest.db");

...
>
> A standalone [`scripts/ensure-nuget-writable.sh`](scripts/ensure-nuget-writable.sh)
> provides the same probe-then-quarantine recovery for callers that would rather run
> a single script once before an out-of-tree build than go through `./build.sh` or
> MSBuild. As long as you own your home directory it relocates the inaccessible tree
> aside, recreates a user-owned `~/.nuget`, and symlinks the preserved package cache
> back in so restore reuses it instead of re-downloading. It is idempotent — a no-op
> when `~/.nuget` is already writable — so it is safe to run unconditionally at the
> start of a build.
>

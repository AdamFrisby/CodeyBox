...
/// MSBuild project (via the baseline's <c>file → project</c> map) and selects
/// the producer-precomputed affected tests for those projects, plus the tests
/// defined in the changed files themselves.
///
/// <para>Fail-safe: ANY uncertainty — no baseline, unknown changeset, a
/// whole-file change (no line granularity), a changed file the baseline never
/// references, or an empty affected set — resolves to the full suite. Running
/// more tests is always safe; running fewer than the change requires is
/// not.</para>
/// </summary>
...
/// the tests defined in those files). Falls back to
...
public sealed class ProjectGraphTestSelector : ITestSelector
{
    ...
    public TestSelectionDecision Select(TestSelectionRequest request)
    {
        ...
        var resolved = ProjectGraphSelectorCore.SelectTests(request.ChangedFiles, request.Baseline);

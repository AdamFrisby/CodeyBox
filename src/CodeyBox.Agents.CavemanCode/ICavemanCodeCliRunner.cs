...
/// <summary>
/// Runs the <c>caveman-code</c> CLI on the host for probes that need local output.
...
{
    /// <summary>
    /// Runs <c>{binary} --list-models</c> and returns exit code plus captured streams.
    /// </summary>
    ...
    Task<CavemanCodeCliRunResult> RunListModelsAsync(string binary, CancellationToken ct);
}

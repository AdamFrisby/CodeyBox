...
/// <summary>
/// Recognises quota / rate-limit / auth failures emitted by the autohand CLI
/// (verified against autohand-cli 0.9.7 live frames).
///
/// <para>Autohand wraps the backing provider's error in a
/// <c>{"type":"error","message":"…"}</c> stream frame (also echoed to
/// stderr), so the detector scans BOTH streams. Verified shapes:</para>
/// <list type="bullet">
/// <item><description>Bad OpenRouter key (exit 1):
/// <c>Authentication failed. Please verify your OpenRouter API key in
/// ~/.autohand/config.json.\nUser not found.</c></description></item>
/// <item><description>$0-spend-limit key against a paid model (exit 1):
/// <c>Access denied. Your OpenRouter API key may not have permission for
/// this model.\nKey limit exceeded (total limit). …</c></description></item>
/// <item><description>Non-completing command (exit 0):
/// <c>Command did not complete successfully.</c> — a generic failure, NOT a
/// quota signal, deliberately unmatched.</description></item>
/// </list>
///
...
/// patterns via <c>CodeyBox:QuotaFailurePatterns:autohand</c> without
...
/// autohand prompts can contain repository content under review, and model
/// output citing "429" or discussing quota code must not gate dispatch.
/// <c>Command did not complete successfully</c> is intentionally absent — it
/// is the generic give-up shape, not evidence of quota/auth state.</para>
/// </summary>
public sealed class AutohandQuotaFailureDetector : IAgentQuotaFailureDetector
{
    public AgentKind Kind => AgentKind.Autohand;

    /// <summary>
    /// Built-in pattern allowlist. Order matters: the spend-limit rows come
    /// before the access-denied row so the combined paid-model refusal (which
    /// carries BOTH shapes) parks on the hard-cap backoff
    ...
    [
        ...
        // Provider-shaped auth failures (verified: bad OpenRouter key).
        // "User not found" alone is deliberately absent — it matches reviewed
        // prose (user-management code, docs) far more often than provider
        // auth state; the anchored sentence below carries the signal.
        new("Authentication failed", QuotaFailureKind.Unauthorized),
        new("verify your OpenRouter API key", QuotaFailureKind.Unauthorized),
    ...
    /// </summary>
    public AutohandQuotaFailureDetector() : this(additionalPatterns: null) { }
    ...
    /// </summary>
    public AutohandQuotaFailureDetector(IEnumerable<QuotaFailurePattern>? additionalPatterns)
    {

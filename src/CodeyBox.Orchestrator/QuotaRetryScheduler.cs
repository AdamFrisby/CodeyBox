...
public sealed class QuotaRetryScheduler : BackgroundService, IDisposable
{
    ...
    public async Task NotifyQuotaFailureAsync(WorkItem item)
    {
        ...
        if (item.State != WorkItemState.Failed || item.FailureKind != "quota") return;
        ...
        var nextRetryAt = resetAt.Value.Add(_opts.AutoRetryOnQuotaFailure.ClockDriftSafetyMargin);

...
public sealed class QuotaGatePolicy
{
        ...
        string? poolName)
    {
        ...
        // Safety: an operator-configured reserve floor (per-agent
        // <c>FloorByAgent</c> or per-pool <c>FloorByPool</c>) is explicit
        // intent that must not be silently bypassed when the probe cannot
        // produce a reading. Fail CLOSED to protect the reserve rather than
        // letting the UnknownPolicy fail open and dispatch below the reserve.
        // Members WITHOUT an explicit reserve keep the existing UnknownPolicy
        // behaviour.
        if (HasExplicitReserveFloor(options, member))
            return new QuotaGateDecision(
                false,
                "quota unknown; explicit reserve floor configured; fail-closed to protect the reserve",
                floor,
                PoolId: poolName);

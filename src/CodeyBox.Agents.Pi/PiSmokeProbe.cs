...
/// contains a non-empty <c>ANTHROPIC_API_KEY</c> (the shipped credential
/// mapping wires host <c>CODEYBOX_PI_API_KEY</c> to it); returns Fail
/// otherwise.
///
...
/// runner lifts the terminal error (see <see cref="PiTerminalDiagnoser"/>).
/// Mirrors the Cursor/Opencode presence-check probes.</para>
/// </summary>
public sealed class PiSmokeProbe : IAgentSmokeProbe
{
    private readonly ILogger<PiSmokeProbe>? _log;

    public AgentKind Kind => AgentKind.Pi;

    public PiSmokeProbe(ILogger<PiSmokeProbe>? log = null)
    {
        _log = log;
    }

...
| `ManifestPath` | — | `--manifest-path` when the crate under audit is not at the repository root — set it here **or** as a `--manifest-path` flag in `ExtraArguments` (setting both is a deterministic failure). Whichever channel supplies it, the root `Cargo.toml` presence check is skipped and the suppression probe below inspects the pointed-to manifest's workspace — the same manifest the scan consults. |
| `TrustRepositorySuppression` | `false` | When `true`, `[package|workspace.metadata.cargo-semver-checks]` lint tables in the audited repository are honored. When `false`, the run fails closed as deterministic infrastructure whenever `cargo metadata` reports a `cargo-semver-checks` table under any workspace member's `package.metadata` or the `workspace.metadata` — a semantic check over cargo's own normalized manifest output, so quoted, whitespace-dotted, `\u`-escaped, or inline-table TOML spellings cannot evade it, and vendored/fixture manifests the tool never consults cannot trip it. The fixed token is searched across the **entire** normalized metadata document, so any occurrence fails closed — not just a lint table: a workspace member, dependency, or target named `cargo-semver-checks`, the substring in a package's `description`/`keywords`/`categories`, or a checkout path containing it all trip the gate the same way — set this key to trust such a repository. |

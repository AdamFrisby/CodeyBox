...
/// <summary>
/// Builds the HTTP client the 1Password plugin sends credential-bearing
/// traffic through. The client never follows redirects: a 3xx from Connect
/// is untrusted runtime output, and following it would re-send the
/// Authorization bearer token to the redirect target. Redirects therefore
/// surface to each send site as ordinary 3xx responses, which refuse them
/// explicitly.
/// </summary>
internal static class OnePasswordHttpClients
{
    /// <summary>A client for Connect API traffic. Never follows redirects.</summary>

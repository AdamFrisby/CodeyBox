...
VNC service allows only the host bridge gateway. For operator viewing, run
`codeybox-vnc-loopback <multipass-vm-name> 5901` and connect to
`127.0.0.1:5901`. The CodeyBox screenshot/input APIs call `scrot` and
`xdotool` through `multipass exec`; no additional LLM network surface is
required.

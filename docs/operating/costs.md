...
| `aider` | yes, for the shipped OpenRouter-backed member | aider is a multi-provider front routed through litellm; the bundled bucket mirrors the OpenRouter list price for `openrouter/anthropic/claude-haiku-4.5`, keyed by the full dispatch id from aider's `Model:` run header. Cost attribution is an ESTIMATE (cache-write tokens ride inside aider's sent total; OpenRouter may add margin). Operators fronting other providers add that provider's list prices under `CodeyBox:AgentPricing`. |
...
| `prime` | yes, for the shipped OpenRouter-backed member | prime-agent is a multi-provider front; the bundled bucket mirrors the OpenRouter list price for `anthropic/claude-haiku-4.5`, keyed by the verbatim `message.model` id (plus a $0 row for the `:free` smoke tier). Cost attribution is an ESTIMATE (OpenRouter may add margin). Operators fronting other providers add that provider's list prices under `CodeyBox:AgentPricing`. |
| `autohand` | yes, explicit $0 for the shipped free-tier member — but cost extraction reports unknown, never zero | autohand is a multi-provider front; the bundled bucket records the shipped OpenRouter `:free` model's zero rate keyed by the dispatched `--model` id. The bare headless stream carries no machine-readable usage frame (only a rounded human footer total with no input/output split), so the extractor returns null — no cost row is recorded — until a build emits usage events. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
| `vibe` | explicit $0 for the shipped free-tier member; per-run cost unknown | vibe is a multi-provider front and its programmatic history entries carry no token counts, so the cost extractor reports unknown and per-run rows fall back to elapsed-time attribution. The bundled bucket records the shipped OpenRouter `:free` model's zero rate (keyed by the provider id behind the guest alias) so the $0 spend is explicit, not a fabricated default. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
| `cline` | yes, explicit $0 for the shipped free-tier member | cline is a multi-provider front; the bundled bucket records the shipped OpenRouter `:free` model's zero rate keyed by the provider-native id from `run_result.model.id`. The terminal `run_result` frame carries machine-readable `usage`/`aggregateUsage`, so the extractor returns a measured snapshot (a failure `run_result` with all-zero usage yields a zero snapshot — nothing was billed); output with no usage frame returns null (unknown), never a fabricated zero. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
| `kilo` | yes, explicit $0 for the shipped free-tier member | kilo is a multi-provider front (an OpenCode fork); the bundled bucket records the shipped OpenRouter `:free` model's zero rate keyed by the full `-m` dispatch id (`openai-compatible/…` qualifier retained — the stream carries no model id, so attribution falls through to the AgentDefaults-derived rate). The `step-finish` frame reports fresh `input`, cached `cache.read`, and `output` (verified: `total` equals their sum plus `reasoning`, which has no cost bucket and is ignored). Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
| `omp` | yes, explicit $0 for the shipped free-tier member | omp is a multi-provider front (oh-my-pi fork of pi); the bundled bucket records the shipped OpenRouter `:free` model's zero rate keyed by the bare `message.model` id (omp strips the `openrouter/` qualifier — verified live). The usage frame reports `input`, cached `cacheRead`, and `output` (`reasoningTokens`/`cacheWrite` have no cost bucket and are ignored); all-zero usage on a failure run yields unknown, never a fabricated zero. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
| `continue` | explicit $0 for the shipped free-tier member; per-run cost unknown | Continue is a multi-provider front and its headless transport carries no token counts, so the cost extractor reports unknown and per-run rows fall back to elapsed-time attribution. The bundled bucket records the shipped OpenRouter `:free` model's zero rate (keyed by the seeded guest-config model id) so the $0 spend is explicit, not a fabricated default. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |
...
| `crush` | explicit $0 for the shipped free-tier member; per-run cost unknown | Crush is a multi-provider front (Charm) and its headless transport carries no token counts, so the cost extractor reports unknown and per-run rows fall back to elapsed-time attribution. The bundled bucket records the shipped OpenRouter `:free` model's zero rate (keyed by the qualified `-m` dispatch id) so the $0 spend is explicit, not a fabricated default. Operators fronting paid models add that model's list prices under `CodeyBox:AgentPricing`. |

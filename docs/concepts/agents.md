...
| `qwen`      | `qwen`            | `OPENAI_API_KEY` (the runner pins `--auth-type openai`; `OPENAI_BASE_URL` and `OPENAI_MODEL` are the endpoint and model fallbacks — see [Qwen quirks](../reference/agent-quirks.md#qwen-code-qwen)) | `CODEYBOX_QWEN_API_KEY` (+ `CODEYBOX_QWEN_BASE_URL`, `CODEYBOX_QWEN_MODEL`) |
...
| `devin`   | `curl -fsSL https://cli.devin.ai/install.sh \| bash` | Installs the `devin` binary at `~/.local/bin/devin`. Driven over ACP (`devin acp`) — session updates stream as NDJSON envelopes. See [Devin quirks](../reference/agent-quirks.md#devin-cli-devin). |
...
| `qwen` | `npm install -g @qwen-code/qwen-code@0.24.0` | Apache-2.0; needs Node.js 22+ on the image. Version-pinned for reproducible bakes — re-verify the headless contract (`--output-format stream-json`, prompt on stdin, no positional prompt) before bumping. See [Qwen quirks](../reference/agent-quirks.md#qwen-code-qwen). |

...
| --- | --- | --- | --- | --- | --- |
| CB-2026-001 | High | The deployed admin UI has application authentication disabled. A Cloudflare policy error, origin bypass, or local foothold exposes operator functionality without a second authorization boundary. | Deployment service configuration: `CodeyBoxAdmin__RequireAuth=false` | Enable real origin-side authentication or validate Cloudflare Access JWTs at the origin with administrator authorization. | Remediated 2026-08-12: origin validates Cloudflare JWT issuer/audience and permits only the configured operator email domain; optional native Google OAuth is supported. |
...
| CB-2026-008 | Medium | CodeQL identified log entries that accepted operator-controlled route, audit, or control values without removing control characters. A malicious value could forge a line in plain-text log output. | GitHub CodeQL alerts #1–#20 (`cs/log-forging`) | Remove CR/LF from untrusted log fields before writing them. | Remediated 2026-08-12: affected admin and audit log fields remove CR/LF before being passed to the logger; the next CodeQL run verifies the alerts are fixed. |
